...
$(() => {
  ...
  function updateTimeBlocks(timeBlocks) {
    ...
    if (timeBlocksElement[0]) {
      timeBlocksElement.html(timeBlocks.map(getTimeBlockElement).join(''));
    }
  }

  function getTimeBlockElement(timeBlock) {
    return `<div data-tb-id="${timeBlock.id}" class="time-block-form">
